...
- `GET https://<your-render-service>.onrender.com/david/songs`
...
### E. Search songs
- **Method**: `GET`
- **URL**: `{{baseUrl}}/david/songs/search/shape`
...
- **Build fails with "file not found" for `Dockerfile`/`pom.xml`**:
  - Set the correct Render `Root Directory`.
- **Connection refused / DB auth failed**:
  - Recheck `DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USERNAME`, `DB_PASSWORD`
- **App boots but tables missing**:
  - Ensure `DDL_AUTO=update` in Render environment variables
- **503 first request**:
